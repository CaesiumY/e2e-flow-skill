...
| `bun.lock` 또는 `bun.lockb` 존재 | bun (Bun 1.2+ 는 텍스트 `bun.lock`, 이전은 바이너리 `bun.lockb`) |
...
Sonnet 초안을 자동 적용하기 전, 하위 모델 특유의 실패를 메인이 걸러낸다. **아래 4종 중 하나라도 실패하면 자동 적용을 차단하고 사용자 검토로 전환한다.**

1. **근거-신호 정합성**: `reasoning`이 인용한 핵심 에러 신호가 Step 3에서 수집한 **실제 Playwright 출력에 존재**하는지 Grep으로 대조한다. 출력에 없는 신호를 인용했다면 **환각 의심** → 차단.
2. **edits 경로 allowlist (신뢰 경계 강제)**: 모든 EDIT 블록의 `file` 이 `e2e/` 하위의 **specs / flows / helpers** 인지 확인한다. 하나라도 앱 소스(`src/`, `app/`, `components/`, `pages/`, `lib/` 등)나 `e2e/mocks/`·fixture 파일을 대상으로 하면 — 분류가 UI_CHANGE / TEST_BUG 로 붙어 있어도 — **차단**한다. self-healer 프롬프트가 이미 이 제약을 두지만 하위 티어 응답이 어길 수 있고, Step 6은 OLD 텍스트가 유일하기만 하면 그대로 적용하므로, **적용 직전인 이 게이트에서 경로를 다시 강제**해야 한다. (allowlist 밖 파일 변경이 필요하다는 판단은 그 자체가 APP_BUG 신호다.)
3. **edits 내용 안전 범위**: UI_CHANGE / TEST_BUG인데 edits가 **단언값(`toBe`/`toContain`)** 을 바꾸면 **APP_BUG 의심** → 차단.
4. **confidence 캘리브레이션**: UI_CHANGE인데 **trace/스크린샷 근거 없이 텍스트 에러만**으로 판단했으면 confidence를 **< 0.5로 취급**한다 (자동 적용 차단).
