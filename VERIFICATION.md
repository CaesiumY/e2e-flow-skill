...
# 다중 에이전트 감사·검증 — v0.4.0

v0.4.0 개정(Opus 오케스트레이터 준비 + 모델 티어링)은 단일 세션 워크스루가 아니라 **다중 에이전트 파이프라인**으로 감사·검증되었다. 오케스트레이터(Fable)는 설계 확정·판정·종합만 담당하고, 감사·구현·검증은 전부 하위 모델(Opus/Sonnet/Haiku) 서브에이전트가 수행했다.

## 방법

| 단계 | 구성 | 결과 |
|---|---|---|
| 1. 감사 | 7차원 병렬 (Opus 4 · Sonnet 2 · Haiku 1): 교차참조 정합 / 오케스트레이션 구조 / 서브에이전트 프롬프트 / Opus 실행 준비도 / 모델 라우팅 / 템플릿 품질 / 외부 문서 | 원시 58건 |
| 2. 통합 | Opus 1 — 중복 병합·자체 반증 | 44건 |
| 3. 적대적 검증 | 발견사항별 Sonnet 2표 반증 시도 | 확정 35 · 이견 5 · 기각 4 |
| 4. 구현 | 파일 소유권 배타 7패키지 병렬 (Opus 3 · Sonnet 3 · Haiku 1) | 16개 파일 개정 |
| 5. 재검증 | 6방향 병렬: 교차참조 재감사 / **Opus 실행자 시뮬레이션 워크스루 3종** (그린필드 Phase 1, Phase 2→3 계약 왕복, Phase 3 직접 진입+Phase 4) / diff 회귀 리뷰 / 앵커 grep | 확정 23건 |
| 6. 수정·재검사 | 6패키지 병렬 수정 + 계약 이음새·앵커 재검사 | 잔존 결함 0 |

## 재검증(5단계)이 잡아낸 대표 결함 — 구현 에이전트가 스스로 보지 못한 것들

| 결함 | 발견 경로 |
|---|---|
| CI filter job 라벨 하드코딩 × 신설 "모든 PR" 트리거 → 워크플로우 전체 스킵 | 교차참조 + 워크스루 + diff 리뷰 3중 독립 발견 |
| `webServer.command: '{{PACKAGE_MANAGER}} dev'` — npm에서 존재하지 않는 명령 | Phase 1 워크스루 |
| 시도 3에서도 edits 적용 → "적용 최대 2회" 불변식 위반 | Phase 2→3 계약 왕복 워크스루 |
| `form.submit('저장')` 명시 인자가 "Helper 기본값 수정" 복구 전략을 무력화 | Phase 2→3 계약 왕복 워크스루 → "값의 출처 추적" 규칙으로 해소 |
| spec 단위 실행 × "테스트별 카운터" 양립 불가 | 다중 실패 시나리오 워크스루 → A4 개정 (spec별 카운터) |
| 라우팅 표의 "부분 재셋업"이 참조하는 절차 부재 (dead reference) | 부분 셋업 변형 워크스루 |

## 판정

✅ 6단계 재검사에서 계약 이음새(phase-2↔3 복귀, phase-3↔healer 토큰 1:1, SKILL↔phase-1 부분 재셋업, CI 치환 짝) 정합 확인, 앵커 grep 잔존 위반 0건. **v0.4.0 release 가능.**

## 검증되지 않은 항목 (v0.4.0)

- **실제 프로젝트에서의 end-to-end 실행** — 이번 검증은 문서·계약 수준의 시뮬레이션 워크스루. 실 타깃 프로젝트 워크스루는 v0.4.x 후속 (v0.2.0 baseline 방식)
- **Self-Healer sonnet 강등의 실제 분류 정확도** — 재검 게이트 3종이 안전망이나, 실측 데이터 없음
- **model 파라미터 미지원 호스트의 폴백 경로** — 문서화만 검증
